...
object LowLevelAPI extends App {
  ...
  val asyncRequestHandler: HttpRequest => Future[HttpResponse] = {
    case HttpRequest(HttpMethods.GET, Uri.Path("/home"), _, _, _) => // method, URI, HTTP headers, content and protocol (HTTP1.1/HTTP2.0)
      Future(HttpResponse(
        StatusCodes.OK, // HTTP 200
        entity = HttpEntity(
          ContentTypes.`text/html(UTF-8)`,
...
//  }
  ...
  val syncExerciseHandler: HttpRequest => HttpResponse = {
    ...
    // path /search redirects to some other part of out website/webapp/microservice
